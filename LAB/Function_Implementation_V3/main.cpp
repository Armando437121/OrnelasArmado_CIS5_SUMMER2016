 ...
 * Created on July 18th, 2016, 12:13 PM
...
int main(int argc, char** argv) {
    ...
    cout<<endl<<"Table of values using an Array"<<endl;
    save9(fvArry,pv,intRate,nComPds);
    display(fvArry,nComPds);
    cout<<endl<<"Table of values using a  Vector"<<endl;
...
void  display(vector<float> &fv,int nYear){
    //Header
...
void  save9(vector<float> &fv,float pv,float intRate,int nYear){
...
void  display(float fv[],int nYear){
    //Header
    cout<<"Year Savings"<<endl;
    //Display data
    cout<<fixed<<setprecision(2)<<showpoint<<endl;
...
void  save9(float fv[],float pv,float intRate,int nYear){
    //Fill the entire array
    for(int year=0;year<=nYear;year++){
        fv[year]=save1(pv,intRate,year);
    }
